        ...
        o.Events = new JwtBearerEvents
        {
            OnTokenValidated = async context =>
            {
                ...
                if (string.IsNullOrWhiteSpace(sub)
                    || !int.TryParse(sub, NumberStyles.Integer, CultureInfo.InvariantCulture, out var userId))
                {
                ...
                int? tokenHome = null;
                if (!string.IsNullOrWhiteSpace(tokenHomeRaw))
                {
                    if (!int.TryParse(tokenHomeRaw, NumberStyles.Integer, CultureInfo.InvariantCulture, out var parsedHome))
                    {
                        context.Fail("Invalid home branch claim.");
                        return;
                    }

                    tokenHome = parsedHome;
                }

                if (tokenHome != user.HomeBranchWarehouseId)
