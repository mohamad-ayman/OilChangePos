...
public class SalesService(IDbContextFactory<OilChangePosDbContext> dbFactory) : ISalesService
{
    public async Task<int> CompleteSaleAsync(CompleteSaleRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (items.Count == 0) throw new InvalidOperationException("يجب أن تحتوي الفاتورة على صنف واحد على الأقل.");
        ...
        if (request.DiscountAmount < 0 || request.DiscountAmount > subtotal)
            throw new InvalidOperationException("قيمة الخصم يجب أن تكون بين صفر وإجمالي الفاتورة.");
    ...
    private static List<SaleItemRequest> NormalizeSaleItems(IEnumerable<SaleItemRequest>? items)
    {
        if (items is null)
            return [];
        ...
        {
            if (item.ProductId <= 0)
                throw new InvalidOperationException("كل سطر بيع يجب أن يحدد صنفاً صالحاً.");
            if (item.Quantity <= 0)
                throw new InvalidOperationException("كمية البيع يجب أن تكون أكبر من صفر في كل السطور.");

            quantitiesByProduct.TryGetValue(item.ProductId, out var existing);
            quantitiesByProduct[item.ProductId] = existing + item.Quantity;
        }
