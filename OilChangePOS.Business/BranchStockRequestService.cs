...
public sealed class BranchStockRequestService(
    IDbContextFactory<OilChangePosDbContext> dbFactory) : IBranchStockRequestService
{
    ...
    public async Task RejectAsync(int adminUserId, int requestId, string? notes, CancellationToken cancellationToken = default)
    {
        ...
        var updated = await db.BranchStockRequests
            ...
            .ExecuteUpdateAsync(s => s
                ...
                .SetProperty(x => x.ResolvedAtUtc, DateTime.UtcNow)
                .SetProperty(x => x.ResolutionNotes, resolutionNotes), cancellationToken);
        if (updated == 0)
            await ThrowPendingRequestRequiredAsync(db, requestId, "رفض", cancellationToken);
    }
        ...
        try
        {
            ...
            var updated = await db.BranchStockRequests
                ...
                .ExecuteUpdateAsync(s => s
                    ...
                    .SetProperty(x => x.ResolvedByUserId, adminUserId)
                    .SetProperty(x => x.ResolvedAtUtc, DateTime.UtcNow)
                    .SetProperty(x => x.ResolutionNotes, (string?)null), cancellationToken);
            if (updated == 0)
                throw new InvalidOperationException("يمكن تنفيذ الطلبات المعلّقة فقط.");
            ...
            await db.BranchStockRequests
                .Where(x => x.Id == requestId)
                .ExecuteUpdateAsync(s => s.SetProperty(x => x.FulfillmentStockMovementId, movementId), cancellationToken);
    ...
    public async Task CancelOwnPendingAsync(int userId, int requestId, CancellationToken cancellationToken = default)
    {
        ...
        var updated = await db.BranchStockRequests
            .Where(x => x.Id == requestId && x.BranchWarehouseId == home && x.Status == BranchStockRequestStatus.Pending)
            .ExecuteUpdateAsync(s => s
                .SetProperty(x => x.Status, BranchStockRequestStatus.Cancelled)
                .SetProperty(x => x.ResolvedByUserId, userId)
                .SetProperty(x => x.ResolvedAtUtc, DateTime.UtcNow)
                .SetProperty(x => x.ResolutionNotes, "ألغاه الطالب"), cancellationToken);
        if (updated > 0)
            return;
    ...
    private static async Task ThrowPendingRequestRequiredAsync(
        OilChangePosDbContext db,
        int requestId,
        string action,
        CancellationToken cancellationToken)
    {
        if (!await db.BranchStockRequests.AsNoTracking().AnyAsync(x => x.Id == requestId, cancellationToken))
            throw new InvalidOperationException("الطلب غير موجود.");
        throw new InvalidOperationException($"يمكن {action} الطلبات المعلّقة فقط.");
    }
