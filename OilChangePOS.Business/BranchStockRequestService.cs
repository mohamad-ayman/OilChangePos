    ...
    IDbContextFactory<OilChangePosDbContext> dbFactory) : IBranchStockRequestService
{
    ...
    public async Task RejectAsync(int adminUserId, int requestId, string? notes, CancellationToken cancellationToken = default)
    {
        ...
        var affected = await db.BranchStockRequests
            ...
            .ExecuteUpdateAsync(s => s
                ...
                .SetProperty(x => x.ResolvedAtUtc, (DateTime?)DateTime.UtcNow)
                .SetProperty(x => x.ResolutionNotes, string.IsNullOrWhiteSpace(notes) ? null : notes.Trim()),
                cancellationToken);
        if (affected != 1)
        ...
        try
        {
            ...
            var claimed = await db.BranchStockRequests
                ...
                .ExecuteUpdateAsync(s => s
                    ...
                    .SetProperty(x => x.FulfillmentStockMovementId, (int?)null),
                    cancellationToken);
            if (claimed != 1)
                throw new InvalidOperationException("يمكن تنفيذ الطلبات المعلّقة فقط.");
            ...
            var linked = await db.BranchStockRequests
                .Where(x => x.Id == requestId && x.Status == BranchStockRequestStatus.Fulfilled && x.FulfillmentStockMovementId == null)
                .ExecuteUpdateAsync(s => s.SetProperty(x => x.FulfillmentStockMovementId, movementId), cancellationToken);
            if (linked != 1)
                throw new InvalidOperationException("تعذر ربط حركة المخزون بطلب التوريد.");
    ...
    public async Task CancelOwnPendingAsync(int userId, int requestId, CancellationToken cancellationToken = default)
    {
        ...
        var affected = await db.BranchStockRequests
            ...
            .ExecuteUpdateAsync(s => s
                ...
                cancellationToken);
        if (affected != 1)
            throw new InvalidOperationException("يمكن إلغاء الطلبات المعلّقة الخاصة بفرعك فقط.");
    }
