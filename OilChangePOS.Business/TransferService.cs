...
public class TransferService(IDbContextFactory<OilChangePosDbContext> dbFactory) : ITransferService
{
    ...
    public async Task<IReadOnlyList<int>> TransferStockBulkAsync(TransferStockBulkRequest bulk, CancellationToken cancellationToken = default)
    {
        ...
        await using var db = await dbFactory.CreateDbContextAsync(cancellationToken);
        var actor = await RbacRules.RequireUserAsync(db, bulk.UserId, cancellationToken);
        if (actor.Role != UserRole.Admin)
            throw new InvalidOperationException("المسؤولون فقط يمكنهم تحويل المخزون.");
        ...
        foreach (var l in lines)
        {
            ...
            else
            {
                map[l.ProductId] = cur with
                {
                    ...
                    BranchSalePriceForDestination = MergeBranchSalePrice(cur.BranchSalePriceForDestination, l.BranchSalePriceForDestination)
                };
            }
        }
    ...
    {
        if (current.HasValue && next.HasValue && current.Value != next.Value)
            throw new InvalidOperationException("لا يمكن تكرار نفس الصنف بسعر بيع فرع مختلف في التحويل المجمّع.");
        return next ?? current;
    }
