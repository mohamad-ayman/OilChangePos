...
public class TransferService(IDbContextFactory<OilChangePosDbContext> dbFactory) : ITransferService
{
    ...
    private static List<TransferStockBulkLineRequest> NormalizeAndMergeBulkLines(List<TransferStockBulkLineRequest> lines)
    {
        ...
        foreach (var l in lines)
        {
            ...
            {
                if (cur.BranchSalePriceForDestination is { } curPrice
                    && l.BranchSalePriceForDestination is { } nextPrice
                    && curPrice != nextPrice)
                    throw new InvalidOperationException("لا يمكن تكرار الصنف في التحويل المجمّع بأسعار بيع مختلفة لنفس الفرع.");
