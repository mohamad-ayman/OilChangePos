...
public class ServiceOrderService(IDbContextFactory<OilChangePosDbContext> dbFactory) : IServiceOrderService
{
    public async Task<int> CreateOilChangeServiceAsync(OilChangeRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (details.Count == 0)
            throw new InvalidOperationException("يجب أن تحتوي الخدمة على صنف واحد على الأقل.");
    ...
    private static List<SaleItemRequest> NormalizeServiceDetails(IEnumerable<SaleItemRequest>? details)
    {
        ...
        foreach (var detail in details)
        {
            if (detail.ProductId <= 0)
                throw new InvalidOperationException("كل سطر خدمة يجب أن يحدد صنفاً صالحاً.");
            if (detail.Quantity <= 0)
                throw new InvalidOperationException("كمية الخدمة يجب أن تكون أكبر من صفر في كل السطور.");

            quantitiesByProduct.TryGetValue(detail.ProductId, out var existing);
            quantitiesByProduct[detail.ProductId] = existing + detail.Quantity;
        }
